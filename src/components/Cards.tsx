...
const Cards = () => {
    ...
    const getPosts = async () => {
        try {
            const posts = await firestore().collection(USER_POSTS).get(); // Assuming you have a "users" collection

            for (const postDoc of posts.docs) {
                setPosts((prev: any) => [...prev, postDoc.data()])
            }
        }
        catch (error) {
            console.log(error)
            ...
            <View className="bg-[#FE7878] w-full  flex-row h-[279px]   ">
                <ScrollView
...

                    {
                        status.length > 0 ?
                            status.map((item: any) => {
                                return <SecondCard image={item.image} caption={item.caption} />
                            }
                            )
                            : <View className="flex-row  justify-between px-5 p-5 items-center">
                                <Text className="text-[#511760] font-extrabold text-[24px]"> loading...</Text>
                            </View>
